  ...
  for i in range(len(names)):
      with tf.variable_scope(str(i) + "_th_task"):
          ...
          if ADV:
              ...
              "priv_enc_weights", [hidden_size, hidden_size],
              initializer=tf.truncated_normal_initializer(stddev=0.02))))
          ...
          if ADV:
              ...
              "priv_enc_bias", [hidden_size], initializer=tf.zeros_initializer()))
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    total_loss = task_loss + 0.05*discr_loss + 0.1*orth_loss #HYPERPARAMS HERE!!!!!!!!!!!!!!!!
...
def main(_):
  ...
  if FLAGS.do_train: #TRUE
    ...
    train_examples = _my_create_examples(texts_train_list, labels_train_list, len(names), corpus_length= train_size, batch_size = FLAGS.train_batch_size)# list of examples
    eval_examples = _my_create_examples(texts_test_list, labels_test_list, len(names), corpus_length=test_size, batch_size=FLAGS.eval_batch_size)  # list of examples
  ...
  if FLAGS.do_train:
    ...
    file_based_convert_examples_to_features(
            train_examples, label_list, FLAGS.max_seq_length, tokenizer, train_file) # put binary-serialized features to the train_file
